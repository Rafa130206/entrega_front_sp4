"use client";
import { TipoCarro } from "@/app/types";
...
export default function Carros() {
    ...
    const consumindoApiCarros = async () => {
        try {
            const response = await fetch("http://localhost:3000/api/base-carros");
            if (!response.ok) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!placa || !marca || !modelo || !tipo || !ano || !cor) {
    ...
    return (
        <div>
            <div className="flex flex-col items-center mb-6">
                <h1 className='text-center my-6 pt-2 text-[3vw] md:text-4xl lg:text-5xl font-bold'>Cadastre seu carro!</h1>
                <section className='m-5 w-full flex justify-center'>
                    <form onSubmit={handleSubmit} className="flex flex-col w-[25vw] md:w-[300px] lg:w-[440px] bg-gray-200 rounded-lg p-5">
                        <fieldset className="border-none flex flex-col">
                            <legend className="p-4 text-2xl font-bold text-center">Informações do Carro</legend>
